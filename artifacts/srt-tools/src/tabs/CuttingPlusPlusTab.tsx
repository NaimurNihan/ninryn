...
const CutterCard = forwardRef<CutterCardHandle, CutterCardProps>(
  function CutterCard(
    ...
    const runCut = async (): Promise<{ url: string; name: string } | null> => {
      if (!audioFile || !videoFile || audioDuration === null || videoDuration === null) return null;

      const isExtendCase = cutTime !== null && cutTime > 0 && cutTime < videoDuration;
      const isTrimCase = trimAllowed;
      if (!isExtendCase && !isTrimCase) return null;
      ...
      const doWork = async (eng: FFmpeg) => {
        ...
        const out = await eng.readFile(outFile);
        ...
        try {
          ...
          await eng.deleteFile(clip1NoAudio);
          await eng.deleteFile(mergedFile);
          await eng.deleteFile(concatFile);
        } catch {
          /* ignore */
        }
      };
      ...
      <div
        className={`rounded-2xl border-2 bg-white p-4 shadow-sm transition-colors ${
          videoTooLong && trimAllowed
            ...
            ? "border-rose-500 shadow-md shadow-rose-200/50 bg-rose-50/40"
            : mode === "extend"
            ? "border-blue-400 shadow-md shadow-blue-200/50 bg-blue-50/40"
            : highlight
